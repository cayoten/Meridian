...
class Throttler {
    ...
    handleMember(member) {
        ...
        if (guildData.underRaid) {
            ...
            if (guildData.violationLevel >= configuration.maxViolationLevel) {
                ...
                if (channel) {
                    channel.send({content: configuration.warnMessage});
                }
    ...
    punishMember(member) {
        ...
        if (configuration.banOnRaid) {
            member.send({content: `You have been banned for the reason: **${reason}**`}).catch(err => ({}));
            ...
            if (incidents) {
                incidents.send({content: `\`[${utils.epochToHour(Date.now())}]\` :hammer: **${this.client.user.tag}'s anti-raid** has banned **${member.user.tag}** *(${member.id})* \n\`Ban Reason:\` ${reason}`});
            }
        } else {
            member.send({content:`You have been kicked for the reason: **${reason}**`}).catch(err => ({}));
            member.kick(reason).catch(err => ({}));
            let incidents = member.guild.channels.cache.find(x => x.name === "mod-logs");
            if (incidents) {
                incidents.send({content:`\`[${utils.epochToHour(Date.now())}]\` :boot: **${this.client.user.tag}'s anti-raid** has kicked **${member.user.tag}** *(${member.id})* \n\`Kick Reason:\` ${reason}`});
            }
    ...
    doCleanup(guild) {
        ...
        if (((guildData.lastUserJoin + (configuration.forgetTime * 1000)) < Date.now()) && guildData.userJoinMap.size > 0) {
            if (guildData.underRaid) {
                let channel = this.client.channels.resolve(configuration.warnChannel);
                if (channel) {
                    channel.send({content:"Raid marked as ended, the following users were punished:"});
                    let users = [];
                    guildData.userJoinMap.forEach((time, userId, map) => {
                        users.push(`<@${userId}>\n`);
                    });
                    ...
                    for (let i = 0; i < users.length; i++) {
                        channel.send({content:users[i].join("")}); // let's hope not having a raid with over 1k people
                    }
                }
            }
